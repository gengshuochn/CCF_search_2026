def build_ccf_db():
    """
    构建完整的 CCF 2026 推荐期刊与会议数据库
    来源：中国计算机学会推荐国际学术会议和期刊目录第七版（2026年3月更新）
    """
    db = []
        ...
        "计算机体系结构/并行与分布计算/存储系统": {
            ...
            "会议": {
                ...
                "C": [
                    ...
                    ("JCC", "International Conference on JointCloud Computing"),
                ]
            }
        },
        "计算机网络": {
        ...
        "软件工程/系统软件/程序设计语言": {
            "期刊": {
                ...
                "B": [
                    ...
                    ("IETS", "IET Software"),
                    ("IST", "Information and Software Technology Cambridge"),
            ...
            "会议": {
                ...
                "B": [
                    ...
                    ("CSOC", "International Conference on Service Oriented Computing"),
        ...
        "数据库/数据挖掘/内容检索": {
            "期刊": {
                ...
                "B": [
                    ...
                    ("ΑΕΙ", "Advanced Engineering Informatics"),
        ...
        "计算机图形学与多媒体": {
            "期刊": {
                ...
                "C": [
                    ...
                    ("ΜΤΑ", "Multimedia Tools and Applications"),
            ...
            "会议": {
                ...
                "B": [
                    ...
                    ("13D", "ACM SIGGRAPH Symposium on Interactive 3D Graphics and Games"),
                ...
                "C": [
                    ...
                    ("Pacific Vis", "IEEE Pacific Visualization Symposium"),
        ...
        "人工智能": {
            "期刊": {
                "A": [
                    ...
                    ("ΤΡΑΜΙ", "IEEE Transactions on Pattern Analysis and Machine Intelligence"),
        ...
        "人机交互与普适计算": {
            ...
            "会议": {
                ...
                "C": [
                    ...
                    ("IEEE World Haptics Conference", "IEEE World Haptics Conference"),
        ...
        "交叉/综合/新兴": {
            "期刊": {
                ...
                "C": [
                    ...
                    ("JBI", "Journal of Biomedical Informatics Informatics"),
                    ("", "Medical Image Analysis"),
                    ("TI", "IEEE Transactions on Industrial Informatics"),
            ...
            "会议": {
                ...
                "C": [
                    ("ΑΜΙΑ", "American Medical Informatics Association Annual Symposium"),
    ...
    for domain, types in domains.items():
        for item_type in ["期刊", "会议"]:
            if item_type in types:
                for rating, entries in types[item_type].items():
...
def levenshtein_distance(a: str, b: str) -> int:
    ...
    for i, a_ch in enumerate(a, 1):
        current = [i]
        for j, b_ch in enumerate(b, 1):
...
def allowed_abbr_distance(query: str) -> int:
    if len(query) < 3:
        return 0
    if len(query) < 6:
        return 1
...
def get_match_score(entry: dict, query: str, normalized_query: str):
    abbr = entry["abbr"].lower()
    full = entry["full"].lower()
    normalized_abbr = normalize_abbr(entry["abbr"])

    if abbr == query or (normalized_query and normalized_abbr == normalized_query):
        return 0
    if normalized_query and normalized_abbr and normalized_query in normalized_abbr:
        return 1
    if full == query:
        return 2
    if query in full:
    ...
    if normalized_query and normalized_abbr:
        distance = levenshtein_distance(normalized_query, normalized_abbr)
        if distance <= allowed_abbr_distance(normalized_query):
            return 4 + distance

    return None


def search_ccf(query: str):
    """
    搜索 CCF 数据库
    支持缩写、全名、关键词（不区分大小写）
    返回所有匹配项（最多5个），若无则返回 None
    """
    query = query.strip()
    if not query:
        return None
    ...
    q_lower = query.lower()
    q_abbr = normalize_abbr(query)
    matches = []

    for index, entry in enumerate(CCF_DB):
        score = get_match_score(entry, q_lower, q_abbr)
        if score is not None:
            matches.append((score, index, entry))

    matches.sort(key=lambda item: (item[0], item[1]))
    return [entry for _, _, entry in matches[:5]] if matches else None


def main():
    print("CCF 2026 期刊/会议查询系统（支持缩写、全名或关键词）")
    print("输入 'quit' 退出\n")
    ...
    while True:
        ...
        if results:
            ...
            for i, r in enumerate(results, 1):
                print(f"\n[{i}]")
                print(f"  全名: {r['full']}")
                print(f"  缩写: {r['abbr'] if r['abbr'] else '无'}")
                print(f"  类型: {r['type']}")
                print(f"  CCF评级: {r['rating']}")
                print(f"  所属领域: {r['domain']}")
        else:
            print("\n未查询到")
...
# ... (上方保留原来的 build_ccf_db 函数和 CCF_DB 全局变量) ...

def export_to_js():
    """将 CCF 数据库导出为前端可用的 JS 文件"""
    print("正在生成前端数据...")
    # 将 Python 字典转换为 JSON 格式，并拼接到 JavaScript 变量声明中
    js_content = "const ccfData = " + json.dumps(CCF_DB, ensure_ascii=False, indent=2) + ";"

    # 写入文件
    with open("ccf_data.js", "w", encoding="utf-8") as f:
        f.write(js_content)
    print("生成完毕！已在当前目录生成 ccf_data.js")
